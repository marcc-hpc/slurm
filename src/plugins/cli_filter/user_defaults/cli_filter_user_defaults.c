...
cli_string_option_t *get_options_array(cli_type) {
	if (cli_type == CLI_SALLOC)
		return salloc_opt_si;
	if (cli_type == CLI_SBATCH)
		return sbatch_opt_si;
	if (cli_type == CLI_SRUN)
		return srun_opt_si;
	return NULL;
}


int _set_nodes(char *value, int cli_type, void *opt) {
	char *max_nodes = strchr(value, '-');
	if (max_nodes)
		*max_nodes++ = '\0';
	else
		max_nodes = value;

	if (!cli_si_set(value, "nodes_min", opt, cli_type))
		return SLURM_ERROR;
	if (!cli_si_set(max_nodes, "nodes_max", opt, cli_type))
		return SLURM_ERROR;
...
extern int setup_defaults(int cli_type, void *opt) {
	...
	while (!feof(fp) && !ferror(fp)) {
		...
		*value++ = '\0';
		...
		if (_set_default(key, value, cli_type, opt) != SLURM_SUCCESS) {
			error("Failed to set default %s to %s", key, value);
			continue;
		}
	}
