 ...
 */
extern int cli_filter_plugin_pre_submit(int type, void *data) {
	DEF_TIMERS;
	int i, rc;
...
extern int cli_filter_plugin_setup_defaults(int type, void *data) {
	DEF_TIMERS;
	int i, rc;
	...
	slurm_mutex_lock(&g_context_lock);
	for (i = 0; ((i < g_context_cnt) && (rc == SLURM_SUCCESS)); i++)
		rc = (*(ops[i].setup_defaults))(type, data);
	slurm_mutex_unlock(&g_context_lock);
...
extern int cli_filter_plugin_post_submit(int type, uint32_t jobid, void *data) {
	DEF_TIMERS;
	int i, rc;
	...
	slurm_mutex_lock(&g_context_lock);
	for (i = 0; ((i < g_context_cnt) && (rc == SLURM_SUCCESS)); i++)
		rc = (*(ops[i].post_submit))(type, jobid, data);
	slurm_mutex_unlock(&g_context_lock);
